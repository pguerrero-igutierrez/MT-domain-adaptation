...
on the held-out 10 % test split used during training.
...
backtranslated-corpus/eu-literary-trilingual.jsonl  ca2eu
...
-----
    ...
    python 10_evaluate_literary.py --model outputs/literaryv1 --test-file outputs/test_set.json
...
def reconstruct_test_set(literary_weight: int = 1) -> list[dict]:
    ...
    split = int(len(literary) * TRAIN_SPLIT)
    test_samples = literary[split:]
    return test_samples
